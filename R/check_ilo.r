...
if(str_detect(variable, '/version/')){	
	...
	ilo:::init_ilo(-cl)
	ref_config <- ilo$code$cl_col_ind_clv %>% filter(collection %in% unique('ILMS', 'YTH', 'YI', 'CLD'), indicator %in% unique(df$indicator))
...
ilo:::init_ilo(-cl)
	ref_config <- ilo$code$cl_col_ind_clv %>% filter(collection %in% unique('ILMS', 'YTH', 'YI', 'CLD'), indicator %in% unique(df$indicator)) %>% select(indicator)
